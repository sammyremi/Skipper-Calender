...
export default function LittleThingsSection() {
  ...
  return (
    <section className="py-32 px-6 md:px-12 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-0 w-80 h-80 bg-softPink-100/40 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-80 h-80 bg-babyBlue-100/40 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto space-y-16 relative z-10">
        ...
        <div className="text-center max-w-xl mx-auto space-y-3">
          ...
          <p className="text-charcoal-900/60 text-sm font-sans">
          ...
          {LITTLE_THINGS.map((item, idx) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="px-3 py-1 rounded-full bg-babyBlue-50 text-babyBlue-600 text-[10px] uppercase font-sans tracking-widest font-semibold">
                  {item.category}
                </span>
                <Heart className="w-4 h-4 text-softPink-300 group-hover:text-softPink-500 group-hover:fill-softPink-300 transition-colors" />
              </div>

              <p className="font-serif text-xl md:text-2xl text-charcoal-900 leading-snug font-medium">
                "{item.quote}"
              </p>

              <div className="pt-2 text-[11px] font-sans uppercase tracking-widest text-charcoal-900/40 flex items-center gap-1">
